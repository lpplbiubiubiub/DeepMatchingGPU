...
ifeq ($(INCLUDE_CAFFE_LOCATION),1)
	g++ $(OPTFLAGS) -fPIC -L$(CAFFELIB) -L$(CUDALIB) $^ -shared -Xlinker -rpath $(CAFFELIB) -o $@ -lcaffe -lcusparse
else
	g++ $(OPTFLAGS) -fPIC -L$(CAFFELIB) -L$(CUDALIB) $^ -o $@ -lcaffe -lcusparse
endif
